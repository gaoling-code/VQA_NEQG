...
def get_loader(train=False, val=False, test=False, trainval=False):
    ...
    split = VQA(
        ...
        answerable_only=train or trainval,# 仅仅回答
        dummy_answers=test,
    )
...
class VQA(data.Dataset):
    ...
    def __init__(self, questions_path, answers_path, image_features_path, answerable_only=False, dummy_answers=False):
        ...
        if preloaded_vocab: # None
            vocab_json = preloaded_vocab
        else:
            with open(config.vocabulary_path, 'r') as fd:       # vocab.json
                # 保存问题和答案的词汇表的路径
                vocab_json = json.load(fd)
            word2idx, idx2word = cPickle.load(open(config.glove_index, 'rb'))   # dictionary.pkl
        ...
        self.token_to_index = self.vocab['question']    # 试过，不能改
        self.answer_to_index = self.vocab['answer']     # 试过，不能改
        # print('vocab successfully!')

        # q and a
        self.questions = list(prepare_questions(questions_json))
        self.answers = list(prepare_answers(answers_json))
        self.questions = [self._encode_question(q) for q in self.questions] # 试过，不能改
        self.answers = [self._encode_answers(a) for a in self.answers]      # 试过，不能改
        # print('q and a successfully!')

        # v
        self.image_features_path = image_features_path
        self.coco_id_to_index = self._create_coco_id_to_index() # 试过，不能改
        self.coco_ids = [q['image_id'] for q in questions_json['questions']]

        self.dummy_answers= dummy_answers   # test时为true，其他时候为false

        # only use questions that have at least one answer?
        # 只问至少有一个答案的问题?
        self.answerable_only = answerable_only
        if self.answerable_only:
            self.answerable = self._find_answerable(not self.answerable_only)   # 试过，不能改
    ...
    def _load_image(self, image_id):
        ...
        if not hasattr(self, 'features_file'):
            ...
            # 加载h5文件必须在这里完成，而不是在__init__中，
            # 因为当DataLoader为多个作品进行fork时，每个子对象都会使用相同的文件对象，并且失败。
            # 有多个reader使用不同的文件对象是可以的，所以我们只是在这里init。
    ...
    def __getitem__(self, item):
        ...
        question_ids = [q['question_id'] for q in self.questions_json['questions']]

        coco_ids = [q['image_id'] for q in self.questions_json['questions']]
...
def prepare_questions(questions_json):
    ...
    '''用常用的VQA格式,Tokenize and normalize给定question json文件中的question。'''
...
def prepare_answers(answers_json):
    ...
    """ 用常用的VQA格式,Normalize answer json中给定的answers"""
