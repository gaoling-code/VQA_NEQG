...
class Net(nn.Module):
    ...
    def forward(self, v, b, q, v_mask, q_mask, q_len):
        ...
        f1 = features_similarities.cpu().numpy().reshape(config.batch_size, -1)
        np.savetxt("VQA2.0-Recent-Approachs-2018/data/before_RW.csv", f1, delimiter=",")

        P_t_trans = self.rw(features_similarities, v) # [batch=256, num_objs=36, obj_dim=2048]
        v_head = P_t_trans @ v
        f_save2 = v_head.cpu().numpy().reshape(config.batch_size, -1)
        np.savetxt("VQA2.0-Recent-Approachs-2018/data/after_RW1.csv", f_save2, delimiter=",")

        v_head = 0.95 * v + 0.05 * v_head
        f_save3 = v_head.cpu().numpy().reshape(config.batch_size, -1)
        np.savetxt("VQA2.0-Recent-Approachs-2018/data/after_RW2.csv", f_save3, delimiter=",")
...
def CosineSimilarity(vector1):
    ...
    S = torch.matmul(vector1,vector1.transpose(-1,-2))  # v * v.T
...
class RandomWalk(nn.Module):
    ...
    def forward(self, Association_matrix, v):   # similarity, feature
        ...
        q: [batch, qdim]
        """
        alpha = 0.8     # 衡量原始信息与随机游走后的信息的权重
        step = 10        # 随机游走的步数

        # 对角线位置置为0，然后行规一化为概率转换矩阵probabilistic_transfer_matrix
        for i in range(Association_matrix.size(0)): 
            Association_matrix[i].fill_diagonal_(0)
        probabilistic_transfer_matrix = torch.nn.functional.normalize(Association_matrix, p=1, dim=2) # 概率转换矩阵[2560, 36, 36]
        batchsize, k, _ = probabilistic_transfer_matrix.shape
        probabilistic_transfer_matrix_T = probabilistic_transfer_matrix.transpose(1,2)  # 概率转换矩阵的转置 [2560, 36, 36]

        # P0 = probabilistic_transfer_matrix_T  # 行规一化后的关联矩阵（即：概率转换矩阵）
        # P_t = P0
        # 对角矩阵
        P_t = torch.eye(k, k)
        P_t = P_t.unsqueeze(0).expand(batchsize, k, k).cuda()
        P0 = P_t

        # print("RW前：均值、方差、最大值、最小值、求和/（36*36）、关联矩阵")
        # # 均值
        # print(torch.mean(probabilistic_transfer_matrix))
        # # 方差
        # print(torch.var(probabilistic_transfer_matrix))
        # # 最大值 and 最小值
        # print(torch.max(probabilistic_transfer_matrix))
        # print(torch.min(probabilistic_transfer_matrix))
        # #求和/（36*36）
        # print(torch.mul(torch.sum(probabilistic_transfer_matrix), 1/(36*36*2048)))
        # print(torch.sum(probabilistic_transfer_matrix, dim=-1))
        # # print关联
        # print(probabilistic_transfer_matrix)
        ...
        for t in range(step):
            ...
            if y_max < (1e-06):
                        # 
                # v_head = v + 0.1 * P_t.transpose(1,2) @ v
                # print("随机游走的最终迭代次数：",t)
                # print("RW后：均值、方差、最大值、最小值、求和/（36*36）、关联矩阵")
                # # 均值
                # print(torch.mean(probabilistic_transfer_matrix))
                # # 方差
                # print(torch.var(probabilistic_transfer_matrix))
                ...
                # print(probabilistic_transfer_matrix)
                break
        return P_t.transpose(1,2)   # 更新后的特征
